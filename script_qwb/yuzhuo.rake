...
#
out = [[]]
out = [["区域", "采购商所在城市（地址）", "城市", "采购商名称", "客户属性", "订单区域（前五城市）", "历史交易量订单类型（包车）", "历史交易量订单类型（接送机）", "历史交易记录（订单数量）", "历史交易记录（订单交易金额）", "客户活跃度（分级ABC)"]]
bds = Storage::Base::MG_AREA_BD
...
bds.each do |zone ,name|
 ...
 all_company.each do |n|
     ...
     air_count = res.select{|m| m["_id"]["type"] =~ /机/}.map{|m| m["value"]["count"]}.reduce(:+).to_i
     ...
     out << [zone, c.try(:company_address), c_city, n, "", city_sort, car_count, air_count, all_count, all_price, ""]
